...
program.command('ui').description('Start the local Harness operational UI.').option('--loop-config <path>', 'loop configuration path', 'loop.config.yaml').option('--host <host>', 'loopback host to bind', '127.0.0.1').option('--port <port>', 'TCP port (0 selects a free port)', (value: string) => Number(value), 4321).option('--dev', 'proxy the frontend to a Vite dev server (npm run ui:dev) instead of serving the built assets').action(async (command: { readonly loopConfig: string; readonly host: string; readonly port: number; readonly dev?: boolean }) => {
  const server = await startUiServer({ configPath: command.loopConfig, host: command.host, port: command.port, ...(command.dev ? { devServerUrl: 'http://127.0.0.1:5173' } : {}) })
