...
${marker}` }); actions.push('secret-file hold commented') } catch (error) { actions.push(`PR comment failed: ${message(error)}`) }
  ...
  if (reviewPhase && (!prior || prior.status === 'incomplete')) {
    ...
    review = await runCodeReview(ctx.runner, { cli: reviewSettings.cli, repo: config.project.repo, number: pr.number, provider: reviewProvider, model: chosen.model, mode: reviewSettings.mode, ...(reviewSettings.transport ? { transport: reviewSettings.transport } : {}), profile: reviewSettings.profile, votes: reviewSettings.votes, concurrency: reviewSettings.concurrency, minSeverity: reviewSettings.minSeverity, deadlineMs: Math.min(ctx.reviewDeadlineMs, roleSettings.timeoutMs ?? ctx.reviewDeadlineMs), subprocessTimeoutMs: reviewSettings.subprocessTimeoutMs, analysisMaxTokens: reviewSettings.analysisMaxTokens, analysisGlobalMaxTokens: reviewSettings.analysisGlobalMaxTokens, maxCalls: reviewSettings.maxCalls, post: reviewSettings.post, resultFile, cwd: ctx.loaded.root, env: ctx.env })
