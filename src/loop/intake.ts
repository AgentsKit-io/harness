...
export const runIntakeStage = async (input: { readonly loaded: LoadedLoopConfig; readonly runner: CommandRunner; readonly tracker?: TrackerConnector; readonly now?: () => Date; readonly dryRun?: boolean; readonly bus?: LoopEventBus }): Promise<IntakeReport> => {
  ...
  // the whole invocation; a call with no bus of its own (`loop intake`, tests) stays self-sufficient.
  ...
  const flushNotifications = ownsBus ? attachNotifier(bus, { config, runner: input.runner }) : async () => { /* owner flushes */ }
    ...
    for (const source of config.intake.sources) {
      ...
      for (const alert of alerts) {
        ...
        try {
          ...
          appendLoopEvent(loaded.stateDir, { at: now.toISOString(), type: 'intake.filed', issue: issue.identifier, source: source.id, fingerprint, severity: alert.severity }, bus)
          created += 1
...
export const runMaintainStage = async (input: { readonly loaded: LoadedLoopConfig; readonly runner: CommandRunner; readonly tracker?: TrackerConnector; readonly now?: () => Date; readonly dryRun?: boolean; readonly bus?: LoopEventBus }): Promise<MaintainReport> => {
  ...
  const ownsBus = !input.bus
  const bus = input.bus ?? createLoopEventBus()
  if (ownsBus && config.plugins.modules.length) await loadLoopPlugins(loaded.root, config.plugins.modules, bus)
  const flushNotifications = ownsBus ? attachNotifier(bus, { config, runner: input.runner }) : async () => { /* owner flushes */ }
  ...
  try {
    for (const check of config.maintain.checks) {
      ...
      try {
        ...
        appendLoopEvent(loaded.stateDir, { at: now.toISOString(), type: 'maintain.filed', issue: issue.identifier, check: check.id, fingerprint }, bus)
