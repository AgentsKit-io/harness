...
export const assessSlots = (input: SlotInput): SlotAssessment => {
  ...
  if (adaptive < ceiling) reasons.push(`machine pressure capped concurrency at ${adaptive} (${sample.cpuBusyPercent !== undefined ? `cpu busy ${sample.cpuBusyPercent}%, load ${sample.load1PerCpuPercent}%` : `load ${sample.loadAvailable === false ? 'n/a' : `${sample.load1PerCpuPercent}%`}`}, memory ${sample.memoryUsedPercent}%)`)
