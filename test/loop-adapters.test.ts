...
describe('orca dispatch plan', () => {
  ...
  it('builds worktree set argv, parses terminals and send receipts', async () => {
    ...
    expect(parseOrcaSendReceipt({ send: { accepted: true, prompt: { requestId: 'req-2', stages: ['input_accepted'] } } })).toMatchObject({ accepted: true, requestId: 'req-2', stages: ['input_accepted'] })
    expect(parseOrcaSendReceipt({ stages: [{ stage: 'input_accepted' }] }).accepted).toBe(true)
    expect(parseOrcaSendReceipt({ accepted: false, stages: [{ stage: 'input_accepted' }, { stage: 'wait_timeout' }] }).accepted).toBe(true)
