...
it('requires ten normal issues and records every exclusion', () => {
  ...
  expect(assessPilot({ ...base, entries: [{ issueId: 'ABC-1', classification: 'normal', status: 'excluded' }] })).toMatchObject({ decision: 'blocked', reasons: expect.arrayContaining(['ABC-1 is excluded without an auditable reason.']) })
})
