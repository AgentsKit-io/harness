...
it('records an ordered append-only lifecycle log bound to a real verification run', async () => {
  ...
  const events = new FileEventStore(join(root, '.ak-harness', 'verification')).read(planned.runId)
  ...
  expect(new FileEventStore(join(root, '.ak-harness', 'verification')).verify(planned.runId)).toMatchObject({ status: 'verified', eventCount: 6 })
  ...
  const lockPath = join(root, '.ak-harness', 'verification', 'runs', planned.runId, 'events.ndjson.lock')
  writeFileSync(lockPath, 'busy')
  const store = new FileEventStore(join(root, '.ak-harness', 'verification'))
  ...
  unlinkSync(lockPath)
  const lines = readFileSync(join(root, '.ak-harness', 'verification', 'runs', planned.runId, 'events.ndjson'), 'utf8').trim().split('\n')
  expect(lines).toHaveLength(events.length)
  writeFileSync(join(root, '.ak-harness', 'verification', 'runs', planned.runId, 'events.ndjson'), `${lines.slice().reverse().join('\n')}\n`)
  expect(() => new FileEventStore(join(root, '.ak-harness', 'verification')).verify(planned.runId)).toThrow(/invalid or out of order/)
  const first = JSON.parse(lines[0] ?? '{}') as Record<string, unknown>
  const { eventHash: _eventHash, previousHash: _previousHash, ...legacyFirst } = first
  writeFileSync(join(root, '.ak-harness', 'verification', 'runs', planned.runId, 'events.ndjson'), `${JSON.stringify(legacyFirst)}\n${lines.slice(1).join('\n')}\n`)
  expect(() => new FileEventStore(join(root, '.ak-harness', 'verification')).verify(planned.runId)).toThrow(/mixes legacy/)
  writeFileSync(join(root, '.ak-harness', 'verification', 'runs', planned.runId, 'events.ndjson'), `${JSON.stringify(legacyFirst)}\n`)
  expect(new FileEventStore(join(root, '.ak-harness', 'verification')).verify(planned.runId)).toMatchObject({ status: 'legacy', eventCount: 1 })
  writeFileSync(join(root, '.ak-harness', 'verification', 'runs', planned.runId, 'events.ndjson'), `${JSON.stringify({ ...first, payload: { ...(first.payload as Record<string, unknown>), project: 'tampered' } })}\n${lines.slice(1).join('\n')}\n`)
  expect(() => new FileEventStore(join(root, '.ak-harness', 'verification')).verify(planned.runId)).toThrow(/hash chain is invalid/)
})
