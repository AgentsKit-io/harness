...
const config = (profiles: Record<string, unknown>, profile = 'ci'): string => {
  ...
  const path = join(root, '.ak-harness', 'verification.json')
  writeFileSync(path, `${JSON.stringify(value, null, 2)}\n`)
  return path
}

it('resolves inherited overlays deterministically before validation', () => {
  const loaded = loadConfig(config({ base: { budget: { maxDurationMs: 1000 }, runtime: { kind: 'process' } }, ci: { extends: 'base', checkOverrides: [{ id: 'logic', timeoutMs: 5000 }], cleanup: { roots: ['.ak-harness/verification/tmp'] }, runtime: { kind: 'docker' } } }))
  ...
  expect(loaded.config.cleanup?.roots).toEqual(['.ak-harness/verification/tmp'])
